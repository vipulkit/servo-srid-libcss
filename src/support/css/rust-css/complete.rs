...
pub type ComputeFontSizeCb = @fn(parent: Option<&mut ~css_hint>, child: &~css_hint) -> ~css_hint;
...
fn compute_font_size_cb(parent: Option<&mut ~css_hint>, size: Option<&mut ~css_hint>) -> css_error {
    let cb: ComputeFontSizeCb =
        |mut parent: Option<&mut ~css_hint>, child: &~css_hint| -> ~css_hint {
                    ...
                    CSS_UNIT_EM | CSS_UNIT_PCT=> {
                        if parent.is_some() {
                            ...
                            ~css_hint {
                                ...
                                content: None,
                                counters: None,
                                length:Some(~css_hint_length { value:new_value as i32, unit:parent.get_mut_ref().length.get_ref().unit}),
                                position: None,
                                color: 0,
                                fixed: 0,
                                integer: 0,
